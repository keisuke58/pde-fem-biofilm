...
newton_step_jit = jax.jit(newton_step)


def integrate_0d(theta, t_final, dt_h):
    ...
    phi_sum = jnp.sum(phi_init)
    phi_init = phi_init * (0.999999 / phi_sum)
    ...
    def step(carry, _):
        g_prev = carry
        g_new = newton_step_jit(g_prev, params)
        return g_new, g_new

    g_final, traj = jax.lax.scan(step, g, jnp.arange(n_steps))
    return g_final, traj


def loss_theta(theta, target_phi):
    dt_h = 1e-5
    t_final = 0.02
    g_final, _ = integrate_0d(theta, t_final, dt_h)
    phi_final = g_final[0:5]
    return jnp.mean((phi_final - target_phi) ** 2)


def main():
    dt_h = 1e-5
    t_final = 0.02
    g_final, traj = integrate_0d(THETA_DEMO, t_final, dt_h)
    phi_final = g_final[0:5]
    print("JAX 0D Hamilton 5-species demo")
    print(f"t_final = {t_final}")
    print("phi_final:", ", ".join(f"{float(v):.4f}" for v in phi_final))
    target_phi = phi_final + jnp.array([0.01, -0.01, 0.0, 0.0, 0.0])
    loss_val = loss_theta(THETA_DEMO, target_phi)
    grad_loss = jax.grad(loss_theta)(THETA_DEMO, target_phi)
    print(f"loss(theta) = {float(loss_val):.6e}")
    print("grad norm =", float(jnp.linalg.norm(grad_loss)))

"""
openjaw_p1_auto_import.py  –  Import OpenJaw Patient-1 STL files as orphan-mesh
                               parts into Abaqus CAE.
...
Produces: OpenJaw_P1_auto_withbiofilm.cae
  Parts  : P1_Mandible, P1_Tooth_23, P1_Tooth_30, P1_Tooth_31, CrownBiofilm, SlitBiofilm
  Each part is a TRI3 orphan-mesh surface (sufficient for bbox queries and
  visual assembly; tetrahedron-meshing of closed surfaces is a separate step).

Fixes vs original version
--------------------------
* ASCII/binary STL auto-detection (heuristic + size check)
* elemShape keyword in p.Element() call  (avoids positional-arg mismatch)
* Optional triangle subsampling (--max-tri N) for large Mandible files
* Robust error handling (missing file, empty mesh)
...
# ---------------------------------------------------------------------------
# STL I/O helpers (duplicated from stl_bbox.py to keep this script self-contained
...
def _iter_binary_triangles(path, max_tri=None):
    """Yield (v1, v2, v3) tuples from a binary STL; stops after max_tri if given."""
    with open(path, "rb") as f:
        f.read(80)
        raw = f.read(4)
        if len(raw) < 4:
...
def import_stl_to_part(model, name, stl_path, max_tri=None):
    ...
    max_tri  : maximum triangle count (uniformly subsampled if exceeded)

    Returns
    -------
    Abaqus Part object (may have 0 elements if file is missing/empty)
    """
...
def build_crown_biofilm(model, bb23, poly_pts=None):
    ...
    if cells:
        ...
        elem_type = mesh.ElemType(elemCode=C3D4)
...
def build_openjaw_p1(max_tri_mandible=200000, max_tri_tooth=None):
    """
    Import Patient-1 Mandible + Teeth 23/30/31 into a new Abaqus model.

    Parameters
    ----------
    max_tri_mandible : triangle limit for the (large) Mandible STL
    max_tri_tooth    : triangle limit per tooth (None = import all)
    """
    model_name = "OpenJawP1"
    ...
    for part_name, stl_path, max_tri in parts_spec:
        ...
        # Assign section only if elements were created
    ...
    inst_map = {
        "P1_Mandible": "P1_Mandible-1",
        "P1_Tooth_23": "P1_Tooth_23-1",
        "P1_Tooth_30": "P1_Tooth_30-1",
        "P1_Tooth_31": "P1_Tooth_31-1",
    }
    for part in imported_parts:
        inst_name = inst_map.get(part.name, part.name + "-1")
    ...
    if bb23:
        ...
        if "CrownBiofilm-1" in a.instances:
            inst_crown = a.instances["CrownBiofilm-1"]
        else:
    ...
    cae_path = os.path.join(base_dir, "OpenJaw_P1_auto_withbiofilm.cae")
    ...
    bbox_path = os.path.join(base_dir, "p1_tooth_bbox_from_cae.json")
...
def _parse_args(argv):
    cfg = {
        "job_name": "OJ_ThreeTooth_WithBiofilm",
        "write_inp": True,
        "run_job": True,
    }
    i = 0
...
def main():
    ...
    job = mdb.Job(
        ...
        model=model_name,
        numCpus=1,
        description="OpenJaw three-tooth with biofilm (crown + slit)",
    )
    if cfg["write_inp"]:
        job.writeInput()
    if cfg["run_job"]:

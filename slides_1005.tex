...
\begin{column}{0.4\textwidth}
  \includegraphics[width=\linewidth]{fig_hamilton_vs_comstat.pdf}
\end{column}
\end{columns}
\end{frame}
...
\begin{tabular}{@{}llrr@{}}
\toprule
Case & \texttt{KEYOPT(1,2)} & Expected $\sigma$ & Got\\
\midrule
elastic, $\alpha{=}0.05$ & 0 / 2 / 3 & $-1.0193\times10^{-4}$ & \textbf{match, all 3}\\
viscous, $\alpha{=}0.05$ & 0 & $-6.9632\times10^{-5}$ & \textbf{match}\\
elastic, $\alpha{=}0.20$ & 0 & $-4.7265\times10^{-4}$ & \textbf{match}\\
viscous, $\alpha{=}0.20$ & 0 & $-1.7950\times10^{-4}$ & \textbf{match}\\
\bottomrule
\end{tabular}
\end{table}
All 4 reference cases \emph{and} the element-formulation sweep (B-bar /
enhanced strain / simplified enhanced strain) match exactly --- no volumetric
locking on this load. First time the growth branch has run end-to-end inside
ANSYS, not just the standalone crosscheck driver.
\end{frame}

% --------------------------------------------------------
\begin{frame}{Two real bugs found along the way (both fixed)}
\begin{enumerate}
  \item \textbf{Link error} (\texttt{for\_alloc\_allocatable\_handle} unresolved) ---
        looked like a compiler-version mismatch (chased installing an older
        Intel Fortran as a dead end). Actual cause: ANSYS's own link script
        was missing one library. One line fixed it, no reinstall needed.
  \item \textbf{Silent viscous-state bug} --- APDL's \texttt{TBDATA} command
        only accepts \textbf{6 values per call}; the deck's 9-value viscous-state
        initializer silently dropped the last 3, leaving the prior state singular
        instead of identity. Harmless for the elastic case, but made every
        \emph{viscous} run silently return the elastic answer instead ---
        deceptive because it looked like a match, not a crash.
\end{enumerate}
\vspace{4pt}
{\scriptsize Full writeup: \texttt{ansys\_usermat/apdl/RUNBOOK.md}. Diagnosed
by instrumenting the UMAT with temporary debug output and tracing exactly what
ANSYS was actually passing to the material at the call site.}
\end{frame}

% --------------------------------------------------------
\begin{frame}{UserElement vs.\ UserMat: narrowed, not settled}
The assignment names \emph{UserElement/UserMat}; this repo has a UserMat only
--- it can't by itself transport the ecology field spatially.
\vspace{4pt}
\begin{itemize}
  \item Read ANSYS's own \texttt{UserElem.F} example end to end: confirmed a
        UserElement can call our \emph{existing, verified} UserMat unchanged
        for the mechanical response (\texttt{KEYANSMAT=1} $\to$
        \texttt{ElemGetMat}) --- so today's verification work transfers either way
  \item What has \emph{no} template in ANSYS's own example: the field-DOF
        residual for the ecology transport itself --- genuinely new code either way
  \item \textbf{Working assumption, pending Oliver/Meisam:} extra nodal DOFs
        inside a UserElement (not precomputed externally, as this repo does today)
\end{itemize}
\vspace{4pt}
{\scriptsize Notes: \texttt{ansys\_usermat/USERELEM\_NOTES.md}. Also still
waiting on Felix's actual implementation from Oliver --- our crosscheck
harness (\texttt{crosscheck.py}) is now generalized to diff against it the
moment it arrives.}
\end{frame}
...
\end{column}
\begin{column}{0.5\textwidth}
  \includegraphics[width=\linewidth]{pde_verify_convergence.pdf}\\
  {\scriptsize \TODO{replace with the FEM residual plot if newer}}
\end{column}
\end{columns}
\end{frame}

...
def simulate_hamilton_1d(
    ...
    def body(carry, _):
        G_local = carry
        G_local = reaction_step(G_local, params)
        G_local = diffusion_step(G_local, params)
        return G_local, G_local

    _, G_traj = jax.lax.scan(body, G, jnp.arange(n_macro))
    G_all = jnp.concatenate([G[jnp.newaxis, :, :], G_traj], axis=0)
    return G_all

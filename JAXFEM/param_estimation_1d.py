...
def generate_data(N=64, T=0.1, dt=1e-4, D_true=0.01, k_true=1.0, noise_std=0.01):
    uT, traj_true = solve_nd(dim=1, N=N, T=T, dt=dt, D=D_true, k=k_true, s0=10.0)
...
def loss_params(params, u_obs, N, T, dt, idx):
    traj_sim = solve_1d_with_params(params, N, T, dt)
...
def main():
    ...
    print("JAXFEM 1D reaction-diffusion parameter estimation")
